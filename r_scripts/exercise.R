...
physical_activity <- read_csv(here("data/priority_6/physical_activity/activity_level_2018_tidy.csv")) %>% 
  # change the order of factor levels by specifying the order of activity_level explicitly (class changed in tidy script but fct was not transfered accross)
  mutate(
    activity_level = factor(activity_level, levels = c("Meets recommendations", "Some activity", "Low activity", "Very low activity"))
  )

# plot Physical activity levels graph in Scotland by age, 2018
